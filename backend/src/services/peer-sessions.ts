...
/**
 * One machine's sessions, kept as a block. The order's source of truth is the
 * peers settings (`PUT /api/peers/order`), so blocks are held whole and sorted
 * at the end — an order kept here would be a second source of truth.
 */
...
interface CachedPeerSessions {
  ...
  /** This machine's own position: it is listed in the peers settings too. */
  localOrder?: number;
}
...
/** A peer saved before `order` existed falls to the end. */
...
/** Sort by the peers settings' order. Ties keep their given order (stable sort). */
...
/** The blocks, with their order. Cached for a few seconds; never fans out twice at once. */
 ...
 * order the peers settings say** (`PUT /api/peers/order`, where this machine is
 * itself one row). "The laptop on top" is that setting's decision — a merge
 * that hard-codes itself first silently overrides an order somebody chose.
 *
...
export async function sessionsWithPeers(
  ...
  const { groups, localOrder } = await peerSessionGroups();
  if (groups.length === 0) return local;
  return sortByPeerOrder([...groups, { order: localOrder, sessions: local }])
