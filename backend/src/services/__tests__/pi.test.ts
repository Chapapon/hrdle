...
import { parsePiTail, PiService, PiSessionStore, piSessionIdFromPath } from '../pi';
...
describe('the session store', () => {
  ...
  test('a thread carries the latest answer and the context it sat in', async () => {
    ...
    const threads = await new PiService(store).getThreadsByIds([ID, 'nope']);
    const thread = threads.get(ID);
    expect(thread?.recap).toBe('2');
    expect(thread?.tokenUsage).toMatchObject({ model: 'gpt-6-astra', contextTokens: 6200, totalOutputTokens: 25 });
...
describe('the tail', () => {
  ...
  test('nothing without an assistant turn', () => {
    expect(parsePiTail(lines('t').slice(0, 4))).toEqual({ tokenUsage: undefined, recap: undefined, recapAt: undefined });
  });
});
